...
class Conv2dStaticSamePadding(nn.Module):
    """Compatibility layer for the original 2021 EfficientDet implementation."""
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, bias=True, dilation=1, groups=1, **kwargs):
        super().__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride=stride, bias=bias, dilation=dilation, groups=groups)
...
class MaxPool2dStaticSamePadding(nn.MaxPool2d):
    def forward(self, x):
        ...
        out_h, out_w = math.ceil(height / stride_h), math.ceil(width / stride_w)
        pad_h = max((out_h - 1) * stride_h + kernel_h - height, 0)
        pad_w = max((out_w - 1) * stride_w + kernel_w - width, 0)
        if pad_h or pad_w:
            x = F.pad(x, [pad_w // 2, pad_w - pad_w // 2, pad_h // 2, pad_h - pad_h // 2])
        return F.max_pool2d(x, self.kernel_size, self.stride, 0, self.dilation, self.ceil_mode, self.return_indices)
