...
def postprocess_with_KP(x, anchors, regression, regression_kp, classification, regressBoxes, clipBoxes, threshold, iou_threshold):
    transformed_anchors = regressBoxes(anchors, regression)
    transformed_anchors = clipBoxes(transformed_anchors, x)
    scores = torch.max(classification, dim=2, keepdim=True)[0]
    scores_over_thresh = (scores > threshold)[:, :, 0]
...
def display_with_kp(preds, imgs, obj_list, imshow=True, imwrite=False):
    for i in range(len(imgs)):
        if len(preds[i]['rois']) == 0:
            continue
        ...
        for j in range(len(preds[i]['rois'])):
            ...
            x0_0 = int(kp[0] * (x2 - x1 + 1) + x1 + 0.5)
            y0_0 = int(kp[1] * (y2 - y1 + 1) + y1 + 0.5)

            x0_1 = int(kp[2] * (x2 - x1 + 1) + x1 + 0.5)
            y0_1 = int(kp[3] * (y2 - y1 + 1) + y1 + 0.5)

            x1_0 = int(kp[4] * (x2 - x1 + 1) + x1 + 0.5)
            y1_0 = int(kp[5] * (y2 - y1 + 1) + y1 + 0.5)

            x1_1 = int(kp[6] * (x2 - x1 + 1) + x1 + 0.5)
            y1_1 = int(kp[7] * (y2 - y1 + 1) + y1 + 0.5)
            ...
            if 'ids' in preds[i].keys():
                ...
                cv2.putText(img, 'id: {}'.format((int)(id)),
                            (x1, y2 - 10), cv2.FONT_HERSHEY_SIMPLEX, 1.3,
                            (0, 0, 255), 3)

            if 'gl' in preds[i].keys():
                gl = preds[i]['gl'][j]
                cv2.putText(img, 'gl: {}'.format(gl),
                            (x1, y2 - 50), cv2.FONT_HERSHEY_SIMPLEX, 1.3,
                            (0, 0, 0), 3)

            if 'disease' in preds[i].keys():
                disease = preds[i]['disease'][j]
                cv2.putText(img, 'disease: {}'.format(disease),
                            (x1, y2 - 90), cv2.FONT_HERSHEY_SIMPLEX, 1.3,
                            (0, 0, 0), 3)

        if imshow:
            if img.shape[1] > 1000:
                img = cv2.resize(img, (1000, int(1000 * img.shape[0] / img.shape[1] + 0.5)))
            cv2.imshow('img', img)
            cv2.waitKey(1)
